...
void Mirror::Initialize(ViewProjection* viewProjection, XMMATRIX* matProjection, WorldTransform& worldTransform) {
	obj = new GameObject3D();
	obj->PreLoadModel("Resources/tofu/tofu.obj");
	obj->PreLoadTexture(L"Resources/tofu/mirror.png");
...
void Mirror::CheckCollision(Ray* ray, int i, float* dis) {
	Collision::CheckRay2Mirror(ray[i], ray[i + 1], frontPlane, dis);
	Collision::CheckRay2Mirror(ray[i], ray[i + 1], backPlane, dis);
	Collision::CheckRay2Mirror(ray[i], ray[i + 1], leftPlane, dis);
	Collision::CheckRay2Mirror(ray[i], ray[i + 1], rightPlane, dis);
	Collision::CheckRay2Mirror(ray[i], ray[i + 1], upPlane, dis);
	Collision::CheckRay2Mirror(ray[i], ray[i + 1], downPlane, dis);
}
